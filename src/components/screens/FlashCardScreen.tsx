...
function FlashCardScreen() {
    ...
    const filteredFlashcards = useMemo(() => {
        ...
        return flashCards.filter((flashCard) => flashCard.question.toLowerCase().includes(filter.toLowerCase()));
    }, [flashCards, filter]);
    ...
    return <div className={styles.flashCardScreen}>
        <div className={styles.topRow}>
            ...
            <div className={styles.btnContainer}>
                <FontAwesomeIcon icon={faFilter} size="2x" onClick={handleToggleSettingsDrawer} />
                <IconLinkButton to="/flashcards/create" icon={faPlus} size="2x" />
            </div>
        </div>
        <div className={`${styles.settingsDrawer} ${drawerOpen ? styles.open : ""}`}>
            <h2>Settings</h2>
